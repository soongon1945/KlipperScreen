...
class Panel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        offset = offset if isinstance(offset, (list, tuple)) and len(offset) >= 3 else [0] * 3
        self.x_offset = self.x_offsetbak = float(offset[0])
        self.y_offset = self.y_offsetbak = float(offset[1])
        self.z_offset = self.z_offsetbak = float(offset[2])
        self.req_speed = 0
        self.oheight = 0.0
    ...
    def create_buttons(self):
        ...
        self.buttons["adjusting_offset"].connect(
            "clicked",
    ...
    def save_offset(self, widget, device):
        ...
        if device == "probe":
            ...
            msg = _("Apply the XYZ offset to the corresponding nozzle?")
            msg += (
                f"\n\nX: {self.x_offset:.3f}  Y: {self.y_offset:.3f}"
                f"  Z: {self.z_offset:.3f}"
            )
        if saved_z_offset:
        ...
        title = _("Save Offset") if device == "offset" else _("Save Z")
        self._gtk.Dialog(title, buttons, label, self.save_confirm, device)
    ...
    def show_buttons_for_state(self):
        ...
        if self.state == "printing":
            ...
            homing_origin = self._printer.get_stat("gcode_move", "homing_origin")
            if isinstance(homing_origin, (list, tuple)) and len(homing_origin) >= 3:
                self.zoffset = float(homing_origin[2])
            ...
            if self._supports_eoffset():
                ...
                # babystep.  Offer its save action only when XYZ really changed.
                secondary_changed = any(
                    abs(current - baseline) > 1e-6
                    for current, baseline in (
                        (self.x_offset, self.x_offsetbak),
                        (self.y_offset, self.y_offsetbak),
                        (self.z_offset, self.z_offsetbak),
                    )
                )
                secondary_widget = (
                    self.buttons["save_offset_e2"] if secondary_changed else Gtk.Label()
                )
                self.buttons["button_grid"].attach(secondary_widget, 4, 0, 1, 1)

        self.content.show_all()

    def _supports_eoffset(self):
        return (
            self._printer.config_section_exists("extruder1")
            and "SET_GCODE_EOFFSET" in self._printer.available_commands
        )
